...
def AUSMp(l):
    ...
    f4 = 0.5 * (cs * Mrf * (r1 * z1 + r2 * z2) - cs * np.abs(Mrf) * (r2 * z2 - r1 * z1))
...
def get_flux_bord(l, i, func_bord):
    ...
    """
    if i == -1:
        q = [l.q[0][i], l.q[1][i], l.q[2][i], l.q[3][i]]
        r1, u1, e1, z1 = l.get_param(q)
        p1 = l.p[i]
        ...
        r2, u2, p2, z2 = func_bord(r1, u1, p1, z1, V)

        y2 = l.powd.psi(z2)
        e2 = l.get_energ(p2, r2, y2)
        H2 = e2 + 0.5 * (u2 ** 2) + p2 / r2
        c2 = l.get_Csound(r2, p2, y2)

    else:
        q = [l.q[0][i], l.q[1][i], l.q[2][i], l.q[3][i]]
        r2, u2, e2, z2 = l.get_param(q)
        p2 = l.p[i]
        H2 = e2 + 0.5 * (u2 ** 2) + p2 / r2
        c2 = l.get_Csound(r2, p2, z2)

        V = l.V[i]

        r1, u1, p1, z1 = func_bord(r2, u2, p2, z2, V)

        y1 = l.powd.psi(z1)
        e1 = l.get_energ(p1, r1, y1)
        H1 = e1 + 0.5 * (u1 ** 2) + p1 / r1
        c1 = l.get_Csound(r1, p1, y1)
    ...
    f4 = 0.5 * (cs * Mrf * (r1 * z1 + r2 * z2) - cs * abs(Mrf) * (r2 * z2 - r1 * z1))
