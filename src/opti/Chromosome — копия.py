...
class ChromoController(object):
    ...
    def get_vec_param_min(self, cr):
        lst = [r.get_vec_val_min() for r in self.normalizable_list]
        return np.array(lst)

    def get_vec_param_max(self, cr):
    ...
    def get_border(self, c0, grad0, inc_f_constr=True):
        v0 = c0['vec_param']
        v_min = self.get_vec_param_min(c0['chromo'])
        v_max = self.get_vec_param_max(c0['chromo'])
        to_maxy = grad0 > 0
        to_miny = grad0 < 0
        l = np.zeros_like(v0)
    ...
    def get_bounds_4scipy(self, chromo):
