...
def mega_foo(p_1, ro_1, u_1, p_2, ro_2, u_2, p_0, kappa, eps_F=1e-2, n_iter_max=100, **kwargs):
    ...
    Returns:
        tuple -- UD_left, UD_right, D_1, D_star_1, U, D_star_2, D_2, R_1, R_2, P
                 [0] UD_left   {bool} -- есть ли ударная волна слева
                 [1] UD_right  {bool} -- есть ли ударная волна справа
                 [2] D_1      {float} -- скорость левой границы
                 [3] D_star_1 {float} -- скорость хвоста ВР слева (если ВР слева, если УД -- D_star_1=D_1)
                 [4] U        {float} -- скорость контактного разрыва
                 [5] D_star_2 {float} -- скорость хвоста ВР справа (если ВР справа, если УД -- D_star_2=D_2)
                 [6] D_2      {float} -- скорость правой границы
                 [7] R_1      {float} -- плотность слева контактного разрыва
                 [8] R_2      {float} -- плотность справа контактного разрыва
                 [9] P        {float} -- давление в зоне контактного разрыва
    """
    ...
    if not reverse:
        return UD_left, UD_right, D_1, D_star_1, U, D_star_2, D_2, R_1, R_2, P
    else:
        return UD_right, UD_left, -D_2, -D_star_2, -U, -D_star_1, -D_1, R_2, R_1, P


def plot_rays(show=True, **init_cond):
    import matplotlib.pyplot as plt
    UD_left, UD_right, D_1, D_star_1, U, D_star_2, D_2, R_1, R_2, P = mega_foo(**init_cond)
