...
from gaslayer import GasLayer, GasEOS, GridStrecher, GasFluxCalculator,get_final_limited_value
import time
...
def _plot4():
    ...
    xs = np.linspace(-20,30, 10000)
    ys = np.zeros_like(xs)
    for i, x in enumerate(xs):
        ys[i] = get_final_limited_value(x, -5, 15)
    plt.plot(xs, ys)
    plt.show()
